...
PRIVATE int chosen_fit = FIRST; /* current alloc algorithm */
 ...
 *				do_hole_map				     *
 *===========================================================================*/
/* Return information about current free slots in MM. */
PUBLIC int do_hole_map(void)
{
  register struct hole *iter;  /* iterator */
  phys_clicks buffer[NR_HOLES * 2 + 1];  /* array that this function returns */
  phys_clicks count;  /* count of pairs to return */
  int idx = 0;  /* current index of the buffer array */
  
  count = mm_in.m1_i1 / (sizeof(phys_clicks) * 2); /* nbytes divided by 2*sizeof(phys_clicks) */
  
  /* Iterate over holes and append buffer with size, address of the slot. */
  for (iter = hole_head;
       idx < count && iter != NIL_HOLE && iter->h_base < swap_base;
       iter = iter->h_next) {

    buffer[2*idx] = iter->h_len;  /* size of the slot */
    buffer[2*idx + 1] = iter->h_base;  /* address of the slot */
    idx += 1;
  }

  /* Copy buffer to the user space. */
  if (mm_in.m1_i1 > sizeof(buffer)) {
    buffer[2*idx] = 0;  /* end of the buffer array */
    sys_copy(MM_PROC_NR, D, (phys_clicks) buffer, who, D,
    (phys_clicks) mm_in.m1_p1, (phys_clicks) sizeof(buffer));
  } else {
    if (count % 2 == 0) {
      /* If number of pairs is even there is no room for last 0,
       * replace last pair with 0s.
       */
      idx -= 1;
      buffer[2*idx] = 0;
      buffer[2*idx + 1] = 0;
      ...
      (phys_clicks) mm_in.m1_p1, (phys_clicks) mm_in.m1_i1);
       ...
       /* If number of pairs is odd, there is room for last 0. */
        buffer[2*idx] = 0;  /* end of the buffer array */
        sys_copy(MM_PROC_NR, D, (phys_clicks) buffer, who, D,
	    (phys_clicks) mm_in.m1_p1, (phys_clicks) mm_in.m1_i1);
    }
  ...
  return idx;  /* return number of pairs */
}

/*===========================================================================*
 *				do_worst_fit				     *
 *===========================================================================*/
/* Choose the algorithm which allocates memory,
 * (int) 0 - first_fit,
 * (int) 1 - worst_fit.
 */
PUBLIC int do_worst_fit(void)
{
  int choice = mm_in.m1_i1;  /* get int from the user space */

  /* If user passed undefined int return -1 */
  if (choice != FIRST && choice != WORST)
    return -1;

  chosen_fit = choice; /* set alloc algorithm to given choice.*/
  return 0;
}
/*===========================================================================*
 *				alloc_mem				     *
 ...
 */
  ...
  register struct hole *hp, *prev_ptr, *biggest;
  phys_clicks old_base;

  if (chosen_fit == FIRST) {
    /* Perform first_fit algorithm. */
  do {
	hp = hole_head;
  ...
  return(NO_MEM);
    ...
    /* Perform worst_fit algorithm. */
  do {
        hp = hole_head;  /* first hole */
        biggest = hp;  /* best hole */

        /* Traverse through memory and find biggest hole. */
        while (hp != NIL_HOLE && hp->h_base < swap_base) {
          if(hp->h_len > biggest->h_len) {
            /* If length of current hole is bigger than biggest, replace the biggest. */
            biggest = hp;
          }
        ...
        if (biggest->h_len >= clicks) {
          /* The biggest hole is big enough, use it. */
